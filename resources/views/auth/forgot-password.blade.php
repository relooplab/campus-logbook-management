...
@section("guest-content")
    <h2 class="text-lg font-semibold mb-4 text-text-primary">Lupa Kata Sandi</h2>
    ...
    @if (session("status"))
        <div class="mb-4 px-3 py-2 rounded-xl bg-brand/10 text-brand text-sm">
            {{ session("status") }}</div>
    @endif
    @if ($errors->any())
    ...
    <form method="POST" action="{{ route("password.email") }}" class="space-y-4"> @csrf <div> <label
                ...
                id="email" required value="{{ old("email") }}" autofocus autocomplete="email" placeholder="Email, NIM, atau NIDN"
            ...
            class="w-full rounded-xl bg-brand hover:bg-brand-hover text-[#0b1420] py-2 text-sm font-semibold transition-colors">Kirim
            Tautan
