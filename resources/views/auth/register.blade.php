...
@section("guest-content")
    ...
    <form method="POST" action="{{ route("register") }}" class="space-y-4"> @csrf <input type="hidden" name="role"
        ...
        {{-- ===== Opsi khusus dosen (NIDN) ===== --}}
        <div id="dosen-directory-section" class="hidden space-y-2 border-t border-border pt-3">
            <p class="text-sm font-medium text-text-primary">Data Dosen</p>
            <p class="text-xs text-text-secondary">Data instansi (perguruan tinggi, fakultas, departemen, prodi) dapat dilengkapi nanti di halaman Profil.</p>
            <div>
            ...
            function setRole(role) {
                ...
                if (roleHint) {
                    roleHint.textContent = role === 'dosen'
                        ? 'Daftar sebagai dosen. Anda akan diarahkan ke halaman profil untuk melengkapi data instansi.'
