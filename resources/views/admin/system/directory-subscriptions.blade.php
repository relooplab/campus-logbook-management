                ...
                <tbody>
                    @forelse ($subscriptions as $sub)
                        <tr class="border-b border-border">
                            <td class="py-3 px-4">
                                <span class="inline-block px-2 py-0.5 rounded-full text-[10px] bg-bg-panel mr-1">{{ ucwords(str_replace('_', ' ', $sub->scope_type)) }}</span>
                                <span class="text-text-primary">{{ $sub->scope_id }}</span>
                            </td>
                            ...
                            <td class="py-3 px-4">{{ $sub->ends_at?->format('d M Y') ?? '—' }}</td>
                            <td class="py-3 px-4">
                                @if ($sub->isActive())
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-4 px-4 text-text-secondary">Belum ada langganan direktori.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
