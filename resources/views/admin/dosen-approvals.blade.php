            ...
            <div class="bg-bg-surface rounded-xl border border-border p-5">
                ...
                <div class="flex flex-wrap gap-2">
                    <form method="POST" action="{{ route("admin.approve-dosen.approve", $d) }}"> @csrf <button
                            class="px-4 py-2 rounded-xl bg-brand hover:bg-brand-hover text-[#0b1420] text-sm">Setujui</button>
                    </form>
                    <form method="POST" action="{{ route("admin.approve-dosen.reject", $d) }}"> @csrf <button
                            class="px-4 py-2 rounded-xl bg-status-danger hover:bg-status-danger/90 text-white text-sm">Tolak</button>
                    </form>
                </div>
            </div>
        @endforeach
