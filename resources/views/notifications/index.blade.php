...
<div class="max-w-3xl space-y-4">
    ...
    @else
        <div class="bg-bg-surface rounded-xl border border-border divide-y divide-border">
            @foreach ($notifications as $n)
                @php
                    $unread = $n->unread();
                    $url = data_get($n->data, "url");
                @endphp <a href="{{ route("notifications.show", $n) }}"
                    class="flex items-start gap-3 px-4 py-3 hover:bg-bg-panel hover:bg-bg-hover/50 {{ $unread ? "bg-brand/5" : "" }}">
                    <span
                        class="mt-1.5 h-2 w-2 rounded-full flex-shrink-0 {{ $unread ? "bg-brand" : "bg-bg-hover" }}"></span>
                    <div class="min-w-0">
