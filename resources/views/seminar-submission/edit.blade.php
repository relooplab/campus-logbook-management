...
<div class="max-w-2xl space-y-6">
    ...
    <form method="POST" action="{{ route('seminar-submission.update', $submission) }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="mt-4">
                ...
                <select name="undangan_sebagai" required class="w-full rounded-xl border border-border bg-bg-surface px-3.5 py-2 text-sm">
                    ...
                    @foreach ($undanganOptions as $key => $label)
                        <option value="{{ $key }}" @selected(old('undangan_sebagai', $submission->undangan_sebagai) === $key)>{{ $label }}</option>
                    @endforeach
                </select>
        ...
        <div class="flex flex-wrap gap-2">
            <button type="submit" class="px-4 py-2 rounded-xl bg-brand text-white text-sm font-medium hover:opacity-90">Simpan Perubahan</button>
