    ...
    @if ($hasAff)
        <div class="mt-6 pt-4 border-t border-border">
            ...
            <div class="grid sm:grid-cols-2 gap-2 text-sm text-text-secondary">
                ...
                @endif
                @if ($faculty)
                    <div class="flex items-center gap-2"><span class="material-symbols-outlined icon-sm text-accent-teal">apartment</span> <span>{{ $faculty->name }}</span></div>
                @endif
                @if ($department)
                    <div class="flex items-center gap-2"><span class="material-symbols-outlined icon-sm text-accent-purple">domain</span> <span>{{ $department->name }}</span></div>
                @endif
                @if ($prodi)
                    <div class="flex items-center gap-2"><span class="material-symbols-outlined icon-sm text-accent-purple">school</span> <span>{{ $prodi->name }}</span></div>
                @endif
            </div>
        </div>
    @endif
